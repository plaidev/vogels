...
utils.paginatedRequest = function (self, runRequestFunc, callback) {
  ...
  var responses = [];
  var retry = false;
  var loadCountError = new Error('Load count is over.');
  ...
  var doFunc = function (callback) {
    ...
    runRequestFunc(self.buildRequest(), function (err, resp) {
      ...
      if (self.options.maxLoadCount) {
        if (responses.length > self.options.maxLoadCount) {
          retry = false;
          return setImmediate(callback, loadCountError);
        }
      }
  ...
  var resulsFunc = function (err) {
    if(err && err !== loadCountError) {
      return callback(err);
    }

    return callback(err, utils.mergeResults(responses, self.table.tableName()));
  };
