...
var Query = module.exports = function (hashKey, table, serializer) {
  ...
  this.options = {loadAll: false, maxLoadCount: null};
  this.request = {};
};
...
Query.prototype.maxLoadCount = function (n) {
  this.options.maxLoadCount = n;
...
internals.formatAttributeValue = function (val, docClient) {
  if(_.isDate(val)) {
    return val.toISOString();
  }

  // Store data type(schema) is not necessarily Query data type.
  // but, SDK requires convertion Array, to {String|Number|Binary}Set.
  if(docClient && _.isArray(val) && val.length > 0) {
    var dataType = null;
    if (typeof(val[0])==='string' || val[0] instanceof String) {
      dataType = 'S';
    }
    else if (typeof(val[0])==='number' || val[0] instanceof Number) {
      dataType = 'N';
    }
    // TODO: implements
    //else if (val[0] instanceof(AWS.util.Buffer) || val[0] instanceof(Uint8Array)) {
    //  type = 'B';
    //}

    if (dataType) {
      val = docClient.Set(val, dataType);
    }
  }
...
Query.prototype.buildAttributeValue = function (key, operator) { /* and variable arguments. */
  var docClient = this.table.docClient;

  var values = Array.prototype.slice.call(arguments, 2);
  for (var i=0; i<values.length; i++) {
    values[i] = internals.formatAttributeValue(values[i], docClient);
  }

  return docClient.Condition.apply(docClient, [key, operator].concat(values));
};
